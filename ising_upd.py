...
class TriangularLattice:
    ...
    def metropolis_step(self, T):
        ...
        if delta_energy < 0 or np.exp(-delta_energy / T) > np.random.random():
            ...
            self.update_pair_correlation(i)
            accepted = True
        return accepted
    
    def monte_carlo_loop(self, steps, warmup, T):
        self.pair_correlation = np.zeros_like(self.distances)
        accepted_moves = 0
        ...
        return accepted_moves / steps  
    
    def update_pair_correlation(self, i):
        self.pair_correlation[i, :] = self.magnetic_moments[i] * self.magnetic_moments
        self.pair_correlation[:, i] = self.pair_correlation[i, :]
